...
<?php $alt = lang() === 'fi' ? 'en' : 'fi'; ?>
        <a href="<?= $alt === 'en' ? '/en' . ($page['slug'] ? '/' . $page['slug'] : '') : '/' . ($page['slug'] ?? '') ?>" class="text-muted hover:text-text transition-colors" aria-label="<?= strtoupper($alt) ?>"><?php if ($alt === 'fi'): ?><svg viewBox="0 0 18 12" width="22" height="15" style="display:block;"><rect width="18" height="12" fill="#fff"/><rect x="5" y="0" width="3" height="12" fill="#003580"/><rect x="0" y="4.5" width="18" height="3" fill="#003580"/><rect x="6" y="0" width="1.5" height="12" fill="#fefefe"/><rect x="0" y="5.25" width="18" height="1.5" fill="#fefefe"/></svg><?php else: ?><svg viewBox="0 0 18 12" width="22" height="15" style="display:block;"><rect width="18" height="12" fill="#012169"/><path d="M0,0 L18,12 M18,0 L0,12" stroke="#fff" stroke-width="2.4"/><path d="M0,0 L18,12 M18,0 L0,12" stroke="#C8102E" stroke-width="1.2"/><path d="M9,0 V12 M0,6 H18" stroke="#fff" stroke-width="4"/><path d="M9,0 V12 M0,6 H18" stroke="#C8102E" stroke-width="2.4"/></svg><?php endif; ?></a>
    </nav>
</header>
<nav id="mobile-menu" class="hidden md:hidden border-t border-editorial">
    <div class="max-w-5xl mx-auto px-5 py-4 flex flex-col gap-4">
        ...
        <?php endforeach; ?>
        <?php $alt = lang() === 'fi' ? 'en' : 'fi'; ?>
        <a href="<?= $alt === 'en' ? '/en' . ($page['slug'] ? '/' . $page['slug'] : '') : '/' . ($page['slug'] ?? '') ?>" class="nav-link text-lg text-muted" aria-label="<?= strtoupper($alt) ?>"><?php if ($alt === 'fi'): ?><svg viewBox="0 0 18 12" width="22" height="15" style="display:block;"><rect width="18" height="12" fill="#fff"/><rect x="5" y="0" width="3" height="12" fill="#003580"/><rect x="0" y="4.5" width="18" height="3" fill="#003580"/><rect x="6" y="0" width="1.5" height="12" fill="#fefefe"/><rect x="0" y="5.25" width="18" height="1.5" fill="#fefefe"/></svg><?php else: ?><svg viewBox="0 0 18 12" width="22" height="15" style="display:block;"><rect width="18" height="12" fill="#012169"/><path d="M0,0 L18,12 M18,0 L0,12" stroke="#fff" stroke-width="2.4"/><path d="M0,0 L18,12 M18,0 L0,12" stroke="#C8102E" stroke-width="1.2"/><path d="M9,0 V12 M0,6 H18" stroke="#fff" stroke-width="4"/><path d="M9,0 V12 M0,6 H18" stroke="#C8102E" stroke-width="2.4"/></svg><?php endif; ?></a>
    </div>
</nav>
